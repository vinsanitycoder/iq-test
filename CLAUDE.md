...
- [x] **Phase 10b: Customizable auto-invite email body + URL cap bump** — 27 May 2026. New nullable column `settings.auto_invite_email_body` (migration `supabase/07_auto_invite_email_body.sql`). HR can edit the auto-invite email body via a new textarea in Settings (greyed when auto-invite toggle is off); leaving it blank uses the built-in default. The default itself was replaced with a polished, company-agnostic body tailored for post-IQ context. Also bundled: `interview_video_url` server-side cap raised from 500 → 1000 chars to accommodate Loom-style URLs with embedded tokens. All changes are additive; the InviteEmail template structure (greeting / CTA / deadline footer) is unchanged. Staging tests all passed + 3 production smoke tests passed.
