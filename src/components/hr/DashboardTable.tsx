    ...
    <>
      ...
      {selectedCount > 0 && (
        <div className="mb-3 flex flex-wrap items-center gap-3 bg-fynlo-dark text-white px-4 py-3 rounded-xl">
          ...
          {(() => {
            // Build the ?applicantIds=… query for the selected export buttons
            const selectedApplicantIds = results
              .filter(r => selected.has(r.id) && r.applicants?.id)
              .map(r => r.applicants!.id)
              .join(',')
            return (
              <>
                <a
                  href={`/api/hr/export?applicantIds=${selectedApplicantIds}`}
                  download
                  className="inline-flex items-center gap-1.5 text-xs font-semibold px-3 py-1.5 rounded-lg bg-white/15 hover:bg-white/25 transition-colors whitespace-nowrap"
                  title="Download CSV of selected candidates"
                >
                  <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                ...
                <a
                  href={`/api/hr/export/excel?applicantIds=${selectedApplicantIds}`}
                  download
                  className="inline-flex items-center gap-1.5 text-xs font-semibold px-3 py-1.5 rounded-lg bg-white/15 hover:bg-white/25 transition-colors whitespace-nowrap"
                  title="Download Excel of selected candidates"
                >
                  <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                    <path strokeLinecap="round" strokeLinejoin="round" d="M9 17v-2m3 2v-4m3 4v-6m2 10H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V20a2 2 0 01-2 2z" />
                  </svg>
                  Excel
                </a>
              </>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-gray-100">
                ...
                <th
                  className="text-center px-4 py-3 font-semibold text-fynlo-subtle text-xs uppercase tracking-wide hidden lg:table-cell"
                  title="How long the test took to complete (minutes:seconds, out of 25:00)"
                >
                  Duration
                </th>
